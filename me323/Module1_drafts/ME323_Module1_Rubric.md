...
| **Failure-mode work** | 15% | Flexural yield, average-web shear, the empirical interaction surrogate, the pointwise first-yield comparison, and LTB are interpreted correctly. The student treats the dominant-mode output as a proxy, checks it against failure notes, and identifies at least one disagreement. |
| **GP and acquisition** | 15% | A GP is fit; posterior median, epistemic uncertainty, aleatory noise, and total predictive uncertainty are distinguished. MUI and EI are implemented. The student explains why acquisition uses epistemic sigma. |
