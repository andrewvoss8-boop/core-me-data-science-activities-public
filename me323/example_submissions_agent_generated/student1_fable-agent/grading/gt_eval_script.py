"""STAFF-ONLY. Score student1's committed design on the frozen GT model.
...
(gpr_str_matern_cal), refit on everything. Run from ibeam150_analysis/ (or
...
points = [
    ("STUDENT PICK",             1.39, 13.20),
    ("frozen eq design (b16)",   1.10, 13.25),
    ("frozen gp design (b17)",   1.00, 13.39),
    ("rival: unconstrained acq", 1.00, 13.20),
    ("rival: lane D pick",       1.39, 11.71),
]
print()
...
_, sw_pick = GT.predict([1.39], [13.20])
...
for name, kw in [("Prelab1-cal", dict(sy=66.8e6, k=0.377, tau_i=16.76e6)),
                 ("GT-cal", cal_kw)]:
    Pb, Ps, _, Pl = phys_parts(1.39, 13.20, shear="new", **kw)
    cap, gov = phys_capacity(1.39, 13.20, shear="new", **kw)
