"""STAFF-ONLY. Score student2's committed design on the frozen GT model.
...
Student2 committed to an exact replicate of the frozen equation-query beam
(beam 16), so the "STUDENT PICK" row and the beam-16 row coincide by design.
"""
import sys
import warnings
...
points = [
    ("STUDENT PICK (= beam 16)",   1.10, 13.25),
    ("frozen gp design (b17)",     1.00, 13.39),
    ("acq pick before veto",       1.00, 13.20),
    ("rival: rejected explore",    1.39, 14.88),
...
_, sw_pick = GT.predict([1.10], [13.25])
...
for name, kw in [("Prelab1-cal", dict(sy=66.8e6, k=0.377, tau_i=16.76e6)),
                 ("GT-cal", cal_kw)]:
    Pb, Ps, _, Pl = phys_parts(1.10, 13.25, shear="new", **kw)
    cap, gov = phys_capacity(1.10, 13.25, shear="new", **kw)
