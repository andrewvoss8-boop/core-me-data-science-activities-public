...
available**. A weak decision can produce a strong beam through luck; a strong
...
- **Individual (each student):** Submission 2's section-0 recall answers, written
  ...
  the evidence you'd cite. Dissent costs nothing; unsupported dissent and
...
20 + data/ML 20 + synthesis 25, less the card-row-11 share) with the
...
A submission that cannot answer these is weak, whatever its polish or its
measured strength. A submission that answers all three is strong even if its
beam underperformed.
...
- A reflection that changes the second design in a way the data justifies —
  the Submission 2 section-3 refit is where this shows up concretely.
...
- The preregistration table is the tool for telling reasoning from
  after-the-fact rationalization: check the memo's explanations against what
  the group put on record before the test, and weight the on-record version.
