...
1. Process raw three-point-bend data into strength and strength-to-weight, and record the failure mode. (Exercised twice: Pre-lab 1 §1b reduces four campaign force–displacement traces — including deciding whether the LTB beam's peak is a material strength or a fixture artifact — and each group reduces its own beam's trace after test day.)
...
alternative, card row 8). Individual components: Submission 2's recall answers
...
**Decision card (`ME323_Module1_DecisionCard.md`).** The recurring one-page scaffold — ten rows from "what beam" through "what would change your mind," plus the preregistration table filed before printing. It concentrates the module's reasoning in one place so it stops being dispersed across notebook prompts, and it is the grader's tool for separating reasoning from post-hoc storytelling. Filed with Submission 1 (rows 1–9 + preregistration) and Submission 2 (row 10).
...
- The trace section (Pre-lab 1 §1b), the decision-framing cells in Submission 1, and the four-panel decision map postdate the `run13` builder — fold them into `run13_build_student_notebooks.py` before the next full regeneration or they will be silently dropped.
